// Taken & modified from browserify, which has the MIT license
...
export function utf8Slice(buf: DataView, start: number, end: number): any {
	end = Math.min(buf.byteLength, end);
	let res: any[] = [];

	let i = start;
	while (i < end) {
		let firstByte = buf.getUint8(i);
